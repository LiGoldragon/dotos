...
impl<'input> RoundTripExample<'input> {
    ...
    fn run(&self) {
        ...
        for input in self.inputs {
            let value = TypeReference::from_structural_dotos(input).expect("decode");
            let output = value.to_structural_dotos();
            println!("in  : {input}");
            println!("rust: {value:?}");
            println!("out : {output}");
            assert_eq!(*input, output, "text round-trip mismatch");
            let decoded_output = TypeReference::from_structural_dotos(&output).expect("re-decode");
    ...
    fn print_declaration_order_shadowing(&self) {
        ...
        let correct = TypeReference::from_structural_dotos(input).expect("correct order");
        let rejected =
            MisorderedReference::from_structural_dotos(input).expect_err("rejected order");
        println!("input: {input}");
