...
use nota_next::{
    BlockShape, CaptureName, MacroMatch, PositionPredicate, StructuralMacroNode,
    StructuralMacroNodeError, StructuralVariant,
};
...
impl<'input> RoundTripExample<'input> {
    ...
    fn print_declaration_order_shadowing(&self) {
        println!("== declaration order is the enum variant order ==\n");
        let input = "(Optional Integer)";
        let correct = TypeReference::from_structural_nota(input).expect("correct order");
        let shadowed = MisorderedReference::from_structural_nota(input).expect("shadowed order");
        println!("input: {input}");
        println!("  TypeReference (Optional before Application) -> {correct:?}");
        println!("  Misordered    (Application before Optional) -> {shadowed:?}");
        println!();
        println!("same text, two derived enum types; the enum variant order decides");
        println!("which structural variant wins.");
    }
}
